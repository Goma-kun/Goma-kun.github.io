...
ならべるくん・まもるくん・SmartShot・うながすくん・おさむくん・まとめるくん・独り言・
SmartShot Mac 版・外出前チェック で実施済み。
**未対応: デスクトップ版まもるくん（本人の画面収録待ち）／しゃべるくん（参照音声待ち・静止画でも可）**。

**Mac のデスクトップアプリも画面収録なしで自動収録できる**（2026-08-31 確立、SmartShot Mac 版・
外出前チェックで実施）。Claude アプリに画面収録権限があるので、①PyQt6 の全画面バックドロップで
個人情報を覆う ②クリーンな Chrome（--user-data-dir=一時 + --app=URL）で対象を表示
③ffmpeg avfoundation で録画しつつ Quartz CGEvent でカーソル操作 ④実データはHTML置換で架空化。
スクリプト一式はローカルセッションの scratchpad/demo/（backdrop.py / driver.py / scene*.sh）。

**Chrome 拡張は画面収録なしで自動収録できる**（2026-08-21 確立）。ヘッドレス Chrome（ハブ物は
Chrome for Testing ＋ --load-extension）で拡張の画面を開き、架空のデモデータを流し込み、
疑似カーソルを注入して CDP で操作しながら連続スクショ → ffmpeg で動画化。
収録スクリプトの実物は Obsidian `01_Projects/{Unagasukun,Osamukun,Matomerukun}_素材/` にある。
デモデータは必ず架空のものにする（実データ・実名を映さない）。
